...
parser.add_argument('--val_size', type=int, default=625)
parser.add_argument('--image_size',type = int, default=65)
parser.add_argument('--num_channels',type = int,  default=4)
parser.add_argument('--batch_size',type = int, default=250)
parser.add_argument('--num_epochs', type=int, default=500)
parser.add_argument('--signal_path',type=str, default="../Data/Phantom/clustercalc/")
parser.add_argument('--signal_name',type=str, default="clustercalc_signal")
parser.add_argument('--data_path', type=str, default="../Data/Phantom/clustercalc/")
parser.add_argument('--tied_toggle',type=int, default=1)
parser.add_argument('--validation',type=int, default=1)
parser.add_argument('--train_offset',type=int, default=5000)
parser.add_argument('--val_offset',type=int, default=625)
parser.add_argument('--save_path', type=str, default="../Models/Phantom/clustercalc_signal")
...
#SIGNAL = sio.loadmat(args.signal_path + args.signal_name + '.mat')
#SIGNAL = SIGNAL[args.signal_name]

TRAIN_DATA = sio.loadmat(args.data_path + '/train.mat')
TRAIN_DATA = TRAIN_DATA['train']
#TRAIN_DATA = h5py.File(args.data_path + '/train.mat', 'r')
#TRAIN_DATA = np.transpose(TRAIN_DATA['train'])
...
#VAL_DATA = zscore(VAL_DATA, axis=0)
...
def next_batch(batch_size, count, permutation, offset, data):
  ...
  for i in range(int(batch_size / 2)):
    if count >= (int(len(data)/2)):
      permutation = np.random.permutation(len(data/2))
      count = 0
      #print('Data Shuffled')
      #print(len(data))
    image = data[count]
...
def test_signal():
  ...
  #signal = np.reshape(signal, (IMAGE_SIZE, IMAGE_SIZE), order='F')
  imgplot = plt.imshow(signal)
  plt.show()

def test_image():
  #images = next_batch(2)
  nosigplt = plt.imshow(np.reshape(VAL_DATA[0, :], (IMAGE_SIZE, IMAGE_SIZE), order='F'))
  plt.show()
  sigplt = plt.imshow(np.reshape(VAL_DATA[633, :], (IMAGE_SIZE, IMAGE_SIZE), order='F'))
  plt.show()

def test_image(batch):
  nosigplt = plt.imshow(np.reshape(batch[0][9], (IMAGE_SIZE, IMAGE_SIZE), order='F'))
  plt.show()
  sigplt = plt.imshow(np.reshape(batch[1][9], (IMAGE_SIZE, IMAGE_SIZE), order='F'))
  plt.show()
...
def train():
  ...
  train_step = tf.train.AdamOptimizer(learning_rate=5e-3).minimize(loss)#, aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N, colocate_gradients_with_ops=True)
  ...
  with tf.Session(config=config) as sess:
    ...
    val_permutation = np.random.permutation(len(VAL_DATA))
    train_count = np.inf
    val_count = np.inf
    ...
    while current_step < NUM_EPOCHS:
      ...
      if current_step % 1 == 0:
        ...
        if TIED:
          auc = ske_latent_hotelling_observer(output_images, input_images, labels, W_out, SIGNAL)
        else:
          auc = ske_latent_hotelling_observer(output_images, input_images, labels, W1_out, SIGNAL)
...
def inference(sampled_image):
    if TIED:
      W =_variable_on_cpu(shape=[IMAGE_SIZE * IMAGE_SIZE, NUM_CHANNELS], initializer=tf.truncated_normal_initializer(stddev=5e-6), name=('W'), trainable=True)
      ...
      W1 =_variable_on_cpu(shape=[IMAGE_SIZE * IMAGE_SIZE, NUM_CHANNELS], initializer=tf.truncated_normal_initializer(stddev=5e-6), name=('W1'), trainable=True)
      W2 =_variable_on_cpu(shape=[IMAGE_SIZE * IMAGE_SIZE, NUM_CHANNELS], initializer=tf.truncated_normal_initializer(stddev=5e-6), name=('W2'), trainable=True)
